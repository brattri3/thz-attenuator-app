...
Центральный артефакт. Всё остальное — функции от него.

### 1.1. Как называется файл паспорта — единственно верный формат

Решение владельца П-5 от 2026-08-28:

```
<обозначение прибора>-<серийный номер>.json      например  ATT-11-16-CA85-0042.json
```

* имя файла **совпадает с `device_id` внутри файла**, символ в символ;
* серийный номер — как в бумажном паспорте прибора; пробелы заменяются дефисом;
* один прибор — один файл;
* **даты калибровки в имени нет.**

Приложение кладёт такой файл рядом с собой: `passports/` возле `.exe` либо сам
каталог программы (порядок поиска — `13_CW_APP_RELEASES.md` §2.1).

**Почему дату в имя выносить нельзя — это не вкусовщина.** Поиск паспорта
перечисляет каталог `sorted(folder.glob("*.json"))` (`cwapp/state.py`) и берёт
**первый годный** файл (`cwapp/model.py`). Два паспорта одного прибора с датами
в именах — `ATT-11-16-CA85-0042-2026-03-11.json` и `…-2026-08-14.json` — дадут
лексикографически первый, то есть **старую калибровку**, молча и без единого
предупреждения. Класс дефекта тот же, что у ловушки `_MEIPASS`: арифметика
верная, числа не те. Нужна история калибровок — храните её вне рабочего
каталога программы и кладите рядом ровно один действующий файл.

Если имя файла всё же разошлось с `device_id`, расчёт идёт **по `device_id`**:
он внутри файла и главнее имени. Но расхождение видно в строке состояния —
молча подменять прибор приложение не имеет права.

> Историческая справка: в старом продукте паспорт лежал по пути
> `backend/data/calibrations/<serial>/passport.json`. Этот формат **отменён**
> решением П-5 и в новых поставках не используется.
